...
// --------------------------------------
/**
 * Clear n bits, and set them to 0
 */
IPV6_INT applyBitMask(int n, IPV6_INT out) {
    int i = 0;

    for (; i < n; i++) {
        // i is index of bit from right ( least significant bit)
        // i = 0 is least significant bit
        // i = n-1 is (supposed to be) most significant bit
...
PHPAPI int createNetworkMaskFromPrefix(long prefix, ipv6_range* range TSRMLS_CC) {
    char address[60];
